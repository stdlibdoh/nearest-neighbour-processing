...
public class NNA extends PApplet {
...
class NearestNeighbour
{
  ...
  // Methods
  public int initialise()                                                              // Begins the Algorithm
  {
  ...
  public boolean continueCheck()                                                       // Checks if there's at least 2 values that can be
  ...
  public void findSmallest(int nn)                                                     // Finds smallest number in node currently considered
  {
  ...
  public void rowCrossing(int nn)                                                      // Marks a row as inaccessible after being visited
  {
    helperMatrix[nn] = false;
  }

  public void createHelperMatrix()                                                     // Marks all elements of helperMatrix as no node has been
  {                                                                             // previously visited.
...
class CreateInput
{
  ...
  // Methods
  public int[][] generate()                                                            // Generates the Input
  {
  ...
  public int getSideNo()                                                               // Returns the column number.
  {
    return column;
  }

  public int getWeight(int c, int r)                                                   // Returns the value of an individual cell
  {
    return matrix[c][r];
  }
}
...
class Gui
{
  ...
  public void createDisplay()                                                          // Creates the basic outline
  {
  ...
  public void updateDisplay(int[] m)                                                   // Redraws highlighted cells
  {
    for(int i = 0; i < m.length-1; i++)
    {
      fill(90+2*focus, 76-focus, 146-0.5f*focus);                                // Colour of the highlighted cell
      newCell[m[i+1]][m[i]].createCell(newCell[m[i+1]][m[i]].xPos, newCell[m[i+1]][m[i]].yPos);
      focus += 20;
    }
  }

  public void clearInput()                                                             // Marks the un-highlighted cells
  {
  ...
  public void connectTheDots(int start)                                                // Draws the lines connecting the highlighted cells
  {
  ...
  public void alphaEnabled(int start)                                                  // Marks the highlighted cells
  {
  ...
  public int detectInput()                                                             // Detects position of the mouse and assigns the
  {                                                                             // corresponding starting node to it
    /*if((mouseX > 0 && mouseX < side) && (mouseY > 0 && mouseY < side))
    {
      nodeStart = 0;
    }
    else if((mouseX > side && mouseX < side*2) && (mouseY > side && mouseY < side*2))
    {
      nodeStart = 1;
    }
    else if((mouseX > side*2 && mouseX < side*3) && (mouseY > side*2 && mouseY < side*3))
    {
      nodeStart = 2;
    }
    else if((mouseX > side*3 && mouseX < side*4) && (mouseY > side*3 && mouseY < side*4))
    {
      nodeStart = 3;
    }
    else if((mouseX > side*4 && mouseX < side*5) && (mouseY > side*4 && mouseY < side*5))
    {
      nodeStart = 4;
    }
    else if((mouseX > side*5 && mouseX < side*6) && (mouseY > side*5 && mouseY < side*6))
    {
      nodeStart = 5;
    }
    else if((mouseX > side*6 && mouseX < side*7) && (mouseY > side*6 && mouseY < side*7))
    {
      nodeStart = 6;
    }
    else if((mouseX > side*7 && mouseX < side*8) && (mouseY > side*7 && mouseY < side*8))
    {
      nodeStart = 7;
    }
    return nodeStart;*/
  ...
  public boolean checkPos(int node)
  {
...
class Cell extends Gui
{
  ...
  // Methods
  public void createCell(float x, float y)                                             // Creates the cell
  {
    xPos = x;
    yPos = y;
  ...
  public void settings() {  size(600, 675);  smooth(); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "NNA" };
    if (passedArgs != null) {
